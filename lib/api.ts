...
export async function getNewsPosts(): Promise<any[]> {
  ...
  const query = `*[_type == "newsPost" && ${PUBLISHED_NEWSPOST_FILTER}] | order(dateTime(coalesce(publishDate, _createdAt)) desc) {
    _id, _createdAt, title, slug, "thumbnail": thumbnail.asset->url, category, publishDate, authorName, youtubeUrl, instagramUrl, featured, badge, badgeCustom
  }`;
...
export async function getNewsPostBySlug(slug: string): Promise<any> {
  ...
  const query = `*[_type == "newsPost" && slug.current == $slug${publishedConstraint}][0] {
    _id, _createdAt, _updatedAt, title, slug, "thumbnail": thumbnail.asset->url, "bodyImage": bodyImage{"url": asset->url, alt, caption, credit}, category, publishDate, authorName, youtubeUrl, instagramUrl, featured, badge, badgeCustom,
    content[]{
      ...,
      _type == "image" => {
        ...,

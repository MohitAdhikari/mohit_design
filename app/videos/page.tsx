...
export default async function VideosPage() {
  ...
  if (!videos || videos.length === 0) {
    return (
      <div className="max-w-[1300px] mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 text-center">
        <h1 className="text-3xl md:text-5xl font-black font-space-grotesk tracking-tighter text-gray-900 dark:text-white mb-6">VIDEOS</h1>
        <p className="text-gray-500 dark:text-gray-400 font-mono uppercase tracking-widest text-sm">No videos found. Check back soon.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-[1300px] mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      ...
      {/* HEADER */}
      <header className="mb-10 md:mb-16 border-b border-gray-200 dark:border-gray-800/50 pb-6 flex items-center justify-between">
        <h1 className="text-3xl md:text-5xl font-black font-space-grotesk tracking-tighter text-gray-900 dark:text-white flex items-center gap-3">
        ...
        <Reveal as="section" className="mb-16 md:mb-24">
          <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 items-start">
            <div className="w-full lg:w-2/3">
              <div className="p-1 rounded-2xl bg-gradient-to-b from-red-500/30 via-red-500/10 to-transparent">
                <VideoEmbed 
      ...
      {restVideos.length > 0 && (
        <section>
          <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-800/30 pb-4 mb-8">
            <h2 className="text-xl md:text-2xl font-bold font-space-grotesk uppercase tracking-tight text-gray-900 dark:text-white">
              Latest Uploads
            </h2>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {restVideos.map((video: any, i: number) => (
              <Reveal key={video._id} delay={(i % 3) * 90} className="h-full">
              <div className="group h-full bg-white dark:bg-[#0B0B0F] rounded-2xl border border-gray-200 dark:border-gray-800/40 hover:border-red-400/60 dark:hover:border-red-500/40 transition-all duration-300 overflow-hidden flex flex-col shadow-sm hover:shadow-md hover:-translate-y-1">
                <VideoEmbed 
